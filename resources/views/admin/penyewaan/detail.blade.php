            ...
            <div class="card">
                ...
                <div class="card-body" style="overflow: auto">
                        ...
                        <form action="{{ route('acc',['paymentId' => $detail->first()->payment->id]) }}" method="POST">
                            ...
                            <tr class="{{ ($item->status == 3) ? 'table-danger' : '' }}">
                                ...
                                <td>
                                    ...
                                    <span class="badge bg-secondary">Paket {{ $item->paket }}</span>
                                    @if ($item->status === 3)
                    ...
                    <form action="{{ route('selesai',['id' => $detail->first()->payment->id]) }}" method="POST">
                        @csrf
                        @method('PATCH')
                        <button type="submit" class="btn btn-success mb-4" onclick="javascript: return confirm('Pastikan proses sesi foto sudah selesai dilaksanakan, jika yakin lanjutkan');">Selesai</button>
                    </form>
                    @endif
                    @if ($status != 1)
                          ...
                          <div id="collapseOne" class="accordion-collapse collapse show" aria-labelledby="headingOne" data-bs-parent="#accordionExample">
                            <div class="accordion-body">
                                ...
                                @else
                                    ...
                                    <img src="{{ url('') }}/images/evidence/{{ $detail->first()->payment->bukti }}" alt="" width="500px" style="transition: transform 0.2s; cursor: zoom-in;" onclick="window.open('{{ url('') }}/images/evidence/{{ $detail->first()->payment->bukti }}', '_blank');">
